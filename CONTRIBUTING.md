...
1. **`iac_scanner.__version__` shows `0.0.0+unknown`**
   - **Symptom**: The package version comes from git tags via `setuptools-scm`; a shallow clone or missing tags breaks it.
   - **Fix**: Run `git fetch --tags` or do a full clone, then run `pip install -e .`.

2. **`mypy` errors about `numpy` stub syntax**
   - **Symptom**: `mypy` targets Python 3.12 (see `[tool.mypy]`); using an older interpreter to *run* mypy can surface `type X = ...` parse errors from transitive stubs.
   - **Fix**: Run `mypy` under Python 3.12 or newer.

3. **`ruff format --check` fails on `src/iac_scanner/_version.py`**
   - **Symptom**: That file is generated by `setuptools-scm` and is git-ignored and ruff-excluded. If it shows up and fails format checks, it might have been hand-edited or generated wrongly.
   - **Fix**: Do not hand-edit it. You can safely remove it and let `setuptools-scm` regenerate it on build.
