...
### `iac_scanner.__version__` shows `0.0.0+unknown`

The package version is derived from git tags by setuptools-scm. Shallow clones, missing tags, or editable installs created before tags are available can fall back to `0.0.0+unknown`.

Fetch tags from the repository root and reinstall the editable package:
...
Use `make version` to confirm the tag-derived version setuptools-scm reports.

### `mypy` reports `numpy` stub syntax errors

The mypy configuration targets Python 3.12 syntax. Running mypy under an older interpreter can surface parse errors from transitive stubs that use newer syntax such as `type X = ...`.

Run mypy with Python 3.12 or newer, then rerun the normal type-check command:

```bash
mypy src/iac_scanner
```

### `ruff format --check` fails on `src/iac_scanner/_version.py`

`src/iac_scanner/_version.py` is generated by setuptools-scm and is excluded from formatting. If it appears in your working tree, do not hand-edit it.

Remove the generated file and reinstall from the repository root so setuptools-scm can regenerate it when needed:

```bash
rm -f src/iac_scanner/_version.py
pip install -e ".[dev]"
```
